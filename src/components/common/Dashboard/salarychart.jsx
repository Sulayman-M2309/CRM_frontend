...
export const SalaryChart = ({ balancedata }) => {
  ...
  if (balancedata?.balance && Array.isArray(balancedata.balance)) {
    balancedata.balance.forEach((item) => {
      chartData.push({
        month: item?.expensemonth || "N/A",
        SalriesPaid: item?.totalexpenses || 0,
  ...
  if (chartData.length >= 2) {
    const current = chartData[chartData.length - 1].AvailableAmount;
    const previous = chartData[chartData.length - 2].AvailableAmount;
    if (previous !== 0) {
      trendingUp = Math.round(((current - previous) * 100) / previous);
    }
  }
